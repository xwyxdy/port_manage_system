...
//    }
    ...
    @Override
    public boolean reviewShip(Integer shipId, ReviewQueryDTO query) {
        Integer reviewerId=query.getReviewerId();
        //再检查审核员id
        if(managerMapper.getById(reviewerId)==null){
            throw new BusinessException("管理员"+reviewerId+"不存在");
        }
        ...
        String reviewStatus = query.getReviewStatus();
        boolean b = reviewerMapper.updateReviewerId(shipId,reviewerId);
        boolean r = reviewerMapper.updateReviewStatus(shipId,reviewStatus);
    ...
    @Override
    public ApplicationPageDTO getByReview(ApplicationQueryDTO  query) {
        String reviewStatus = query.getReviewStatus();
        List<InPortApplicationVO> applications = reviewerMapper.getByReview(reviewStatus);
        List<ApplicationResponseDTO> responseList = new ArrayList<>();
        for(InPortApplicationVO application:applications){
            ApplicationResponseDTO response = new ApplicationResponseDTO();
            BeanUtils.copyProperties(application,response);
            response.setShipName(shipMapper.getById(application.getShipId()).getShipName());
            Integer reviewerId=application.getReviewerId();
            if(reviewerId!=null){
                response.setReviewerName(managerMapper.getById(application.getReviewerId()).getUsername());
                responseList.add(response);
            }else{
                responseList.add(response);
            }
        }
        return new ApplicationPageDTO(responseList,responseList.size());
    }
